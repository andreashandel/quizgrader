...
Once you are done filling the student list, add it to the course by clicking the `Add studentlist` button in the UI or manually place it into the `studentlists` folder. **Each student needs to have a `StudentID` and `Password` entry.** Name information is recommended but optional. **The only time the `studentlist` file is checked to ensure it follows the required format is during generation of the grade list, see below.** 
