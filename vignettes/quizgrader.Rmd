...
* `/gradelists` - a folder that will contain the main Excel grade tracking spreadsheets, which is created by combining the student list and quiz information (see below for details).
* `/completequizzes` - a folder which will contain all your quizzes (with answers) once you made them as Excel files. 
...
Once you are done filling the student list, add it to the course by clicking the `Add studentlist` button in the UI or manually place it into the `/studentlist` folder. If you want to keep track of changes, give the file a new name (e.g. `studentlist_new.xlsx`). Any pre-existing file with the same name will be overwritten. You can also manually copy the file into this folder. You can have as many files (student lists and others) in this folder as you like. **When you generate the grade tracking sheet (see below), the most recently modified `.xlsx` file in this folder will be used.** Make sure this file is a valid student list. Ideally, remove all others. All other files will be ignored.

During upload of a new student roster file, it will not be checked for correctness. **The only time this file is checked to ensure it follows the required format is during generation of the grade list, see below.**
...
I recommend only using letters, numbers and underscore for filenames. Other names might work, but since the files are deployed to a remove server (that is likely running Unix/Linux), having file names that contain other characters (e.g. dashes or blank) might cause problems. You can try, it might work `r emo::ji('grin')`. I still recommend not to.
...
A unique ID containing letters/numbers. Only the entry in the first row matters, anything else in further rows is ignored. It cannot be empty. Avoid blank spaces and special characters. They might work, but it's not guaranteed. I also recommend naming the quiz file in accordance with the unique ID (e.g. the ID is `quiz1` and the file is `quiz1.xlsx`) but that is not required.

#### QuestionID

A unique ID for each question. I recommend combining quiz ID with some additional question ID (e.g. `quiz1q1`, `quiz1q2`, etc). Needs to be different for each question.
...
_When to  use `Fuzzy_Integer` or `Integer`:_ If you know the answer is an integer, e.g. if the question is `how many US states are there` and the right answer is `50`, you don't want to allow `49` or `51` to be considered correct, so you should use the `Integer` type. If on the other hand you ask students to do some calculation and the result is `41.50123` and you want them to round it to an integer, if you want to be strict and only allow proper rounding (i.e. `42`) you should use `Integer` type, if you are ok with them making rounding mistakes, you should use `Fuzzy_Integer` which would also accept `41` as correct. A submission of `40.567` would also be considered correct, since the number is first rounded to an integer, then checked if it is within plus/minus 1 of the correct answer.
...
These are any notes/comments you want to add, mostly for yourself. These won't be copied to the student sheets/
...
This can be done manually by just copying the files into the folder. Alternatively, you can press the `Add finished quiz to course` button in the Shiny app. With the shiny app, you need to add one quiz at a time.
...
If you want to make changes to any quiz, just edit, then re-upload through the UI. It will overwrite any previous quiz of the same name. Alternatively, you can directly edit the Excel files in the `completequizzes` folder.

### Remove quizzes

**Currently not working through GUI.**
