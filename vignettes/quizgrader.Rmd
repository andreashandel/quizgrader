...
You are required to provide a name for your course (which will be the name of the main directory). It is recommended you provide a location where you want the course folder and its subfolders to go. If you do not provide that, it will be created in the current working directory. Note that for windows paths, you need to specify paths as follows (double backslash): `C:\\tmp\newcourse`
...
* `/studentlists` - a folder that contains spreadsheets (template and filled ones) with student information. 
* `/gradelists` - a folder that will contain the main grade tracking spreadsheets.
* `/completequizzes` - a folder which will contain all your quizzes (with answers) once you made them. 
* `/studentquizzes` - a folder that will contain quizzes to be given to the students for filling out and submitting. These files are auto-generated once you made your solution sheets.
* `/studentsubmissions` - all individual submissions by students. This is mainly for record keeping and to run some question- and student-specific stats. For easier organization, a sub-folder for each quiz will be created and student submissions organized into those sub-folders.

**Files**
* `studentlist_template.xlsx` is generated template file inside the `/studentlists` folder that should be populated with student information, as described below. 
* `quiz_template.xlsx` is generated inside the `/complete_quizzes` folder. This file should be used as template for creating your own quizzes, as described below. 

If at any point you need a clean version of the templates, you can get them through the user interface (or find them in the `/templates` subfolder of the `quizgrader` package).
...
Get the `studentlist_template.xlsx` file either through the UI or you can find it in the `/studentlists` folder on your computer inside the course folder you just created. 
The template file has several columns that you need to fill: 

* `Lastname` and `Firstname` should be self-explanatory.
...
* Thee `Password` column is there to prevent a student accidentally or purposefully submitting as another student (provided students keep their passwords to themselves). Of course this does not prevent cheating. If students want to, they can share passwords and submit for each other. Or they can share their filled out quiz sheets before submission. **If you are worried about cheating on these kinds of quizzes, this package/setup is likely not suitable!** 

I collect passwords at the beginning of the course through a simple Google form. I copy the passwords manually into the template. If you want to, you can likely automate this process with some R scripts. Disabling the password feature is currently not possible. If you think a setup without a password is a feature I should implement, let me know. For the time being, if you don't want to use this feature, just enter the same password for every student and tell the students what the password is. They'll still have to enter it during submission, but that should not be much of a nuisance.
...
Add it to the system by clicking the `Add studentlist` button in the UI or manually place it into the `/studentlist` folder. You should give it a new name. If you upload it through the UI, it will add the current date/time to the filename so you will have a track record of all versions. If you add manually into the folder, it's your choice how to name.
__When you generate the grade tracking sheet (see below), the most recently modified `.xlsx` file in this folder will be used.__
...
You can get the template by clicking `Get quiz template` in the `quizmanager` menu, or by manually going into the `complete_quiz_sheets` folder of your course.
